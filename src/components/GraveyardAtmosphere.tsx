...
import { playGhostPass, prefersReducedMotion, unlockHauntAudio } from "@/lib/haunt-audio";
...
const MAX_GAP_MS = 58_000;

function HauntPassOverlay({ pass }: { pass: Pass }) {
  return (
    <div className="ga-haunt-layer" aria-hidden="true">
      <div className="ga-haunt-dim" />
      <div
        className={`ga-haunt-pass ga-haunt-${pass.direction}`}
        style={{ top: `${pass.top}%`, ["--haunt-scale" as string]: String(pass.scale) }}
      >
        <svg className="ga-haunt-ghost" viewBox="0 0 64 80" fill="none">
          <path
            d="M32 4c-14 0-24 12-24 28v38c0 2.4 1.8 3.4 3.6 2.2l6-3.6 6 3.6c1.4.8 3.2.8 4.6 0l6-3.6 6 3.6c1.4.8 3.2.8 4.6 0l6-3.6 6 3.6c1.8 1.2 3.6 0 3.6-2.2V32C56 16 46 4 32 4Z"
            fill="currentColor"
          />
          <circle cx="23" cy="30" r="3.4" fill="#fafafa" />
          <circle cx="41" cy="30" r="3.4" fill="#fafafa" />
          <path
            d="M26 44c2.5 3 9.5 3 12 0"
            stroke="#fafafa"
            strokeWidth="1.6"
            strokeLinecap="round"
            opacity="0.55"
        ...
        <span className="ga-haunt-trail" />
      </div>
    </div>
  );
}

/** Fixed graveyard atmosphere  -  mist, plots, and occasional ghost flybys with sound. */
export function GraveyardAtmosphere() {
  ...
  useEffect(() => {
    ...
    const haunt = () => {
      ...
      void unlockHauntAudio().then((ctx) => {
        if (ctx) {
          try {
            playGhostPass(ctx);
          } catch {
      ...
      queue(() => {
        setPass((current) => (current?.id === next.id ? null : current));
        if (!cancelled) {
          queue(haunt, MIN_GAP_MS + Math.random() * (MAX_GAP_MS - MIN_GAP_MS));
        }
      }, 5200);
    };

    queue(haunt, 14_000 + Math.random() * 10_000);
  ...
  return (
    <>
      ...
      {mounted && pass
        ? createPortal(<HauntPassOverlay key={pass.id} pass={pass} />, document.body)
        : null}
    </>
  );
}
