...
import { postAuthPath } from "@/lib/nest/roles";
import { nestMe } from "@/lib/nest/client";

/**
 * Nest redirects here after Google OAuth with ?token=...
 * Sets the httpOnly cookie on the frontend origin, then sends the user onward.
 */
export async function GET(request: Request) {
  ...
  try {
    ...
    const destination = postAuthPath(user, next);
